...
__global__ void mat_transpose_cute_reg_kernel(const T *pA, T *pB, int M, int N,
  ...
  auto mA = make_tensor(make_gmem_ptr(pA),
                        make_layout(make_shape(M, N), GenRowMajor{})); // (M, N), stride = (N,1) at defaut
  auto mB = make_tensor(make_gmem_ptr(pB),
                        make_layout(make_shape(N, M), GenRowMajor{})); // (N, M), stride = (M,1) at defaut

  // 按照尺寸 <BLK_M, BLK_N> 切块，每个block 负责一块！！！
  ...
  Tensor tAgA = local_partition(gA, tA, tx);        // layout 是一个输入为idx 输出也为idx的函数
  Tensor tBgB = local_partition(gB, tB, tx);
  Tensor tAcA = local_partition(cA, tA, tx);

  Tensor tApA = make_tensor<bool>(tAcA.shape(), tAcA.stride());           // 视图：每个thread 负责的； tA是thread的布局
  CUTE_UNROLL
  for (int i = 0; i < size<0>(tApA); i++) {
    CUTE_UNROLL
    for (int j = 0; j < size<1>(tApA); j++) {
      tApA(i, j) = get<0>(tAcA(i, j)) < M && get<1>(tAcA(i, j)) < N;        // get<0> -> 行坐标，get<1> -> 列坐标；确保没超边界！！！
    }
  }
...
void mat_transpose_cute_row2col_reg(torch::Tensor x, torch::Tensor y) {
  ...
  dim3 block(size(tA));                        // CuTE 里 大部分都是1D的block！
